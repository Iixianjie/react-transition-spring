...
import { TransitionProps, TransitionTypes } from './type';
  ...
  bounce: {
    ...
    interpolater({ x, ...props }, toggle) {
      const interp = x
        ...
        .interpolate((_x: any) => `scale3d(${_x},${_x},${_x})`);

      return {
        ...props,
        transform: toggle ? interp : x.interpolate((_x: any) => `scale3d(${_x},${_x},${_x})`),
      };
    },
  },
};

const Transition: React.FC<TransitionProps> = ({ type, alpha, ...props }) => {
  let {
    /* eslint-disable-next-line */
    from, to, interpolater, config, skipFade, changeVisible
...
Transition.propTypes = {
  ...propsTypes,
  type: PropTypes.oneOf(Object.keys(transitionConfigs) as TransitionTypes[]).isRequired,
